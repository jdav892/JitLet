...
// throws if the repository is bare
        if(config.isBare()){
...
// returns the contents of the config file as a nested JS object
        return config.strToObj(files.read(files.jitletPath("config")));
    },

    write: function(configObj){
// stringifies the nested JS object configObj and overwrites the config file with it
        files.write(files.jitletPath("config"), config.objToStr(configObj));
    },

    strToObj: function(str){
// parses the config string str and returns its contents as a nested JS object
        return str.split("[")
